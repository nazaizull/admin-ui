...
const CardGoal = () => {
  ...
  const [isLoading, setIsLoading] = useState(true); // State untuk loader

  const value = (goals.presentAmount * 100) / goals.targetAmount || 0;

  const getData = async () => {
    setIsLoading(true); // Tampilkan loader saat mulai fetch data
    try {
      ...
      setGoals({
        presentAmount: response.data.data[0]?.present_amount || 0,
        targetAmount: response.data.data[0]?.target_amount || 0,
      });

      setIsLoading(false); // Hilangkan loader setelah data berhasil di-fetch
    } catch (error) {
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          console.error("Error fetching goals:", error.response);
        }
      }
    }
  };
      ...
      desc={
        isLoading ? (
          <div className="flex justify-center items-center h-full">
            <div className="loader"></div> {/* Loader animasi */}
          </div>
        ) : (
          <div className="p-2">
